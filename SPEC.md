...
Normalize operation timings to a calibration benchmark in the same job. Absolute operation timings in §10.3/§11.7 are reference values, not user guarantees. Compare the median of five runs to the latest versioned baseline captured on the same runner CPU family; regressions over 10% fail. An unrecognized CPU family fails closed until a reviewed baseline is recorded. Compare cumulative drift only within that hardware series; three consecutive increases fail even when each increase is below 10%. Store raw runs, calibration, environment metadata, selected baseline, and baseline history.

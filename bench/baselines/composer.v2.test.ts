...
import { composerBenchmarkBaseline } from "./composer.v2.ts";

describe("Composer benchmark baseline", () => {
  it("pins five calibrated runs, the browser toolchain, and every continuity workload", () => {
    expect(composerBenchmarkBaseline).toMatchObject({ schema: 2, measurement: "cdp-task-duration-thread-ticks-v1", runner: "ubuntu-24.04", playwright: "1.63.0", runs: 5, maximumRegression: 0.1 });
    ...
    expect(Object.values(composerBenchmarkBaseline.history[0]?.normalized ?? {}).every(value => Number.isFinite(value) && value > 0)).toBe(true);
  });
});

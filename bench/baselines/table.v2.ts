...
export const tableBenchmarkBaseline: TableBenchmarkBaseline = Object.freeze({
  ...
  history: Object.freeze([
    Object.freeze({
      ...
      runnerClass: "local:AMD Ryzen Threadripper 9960X 24-Cores",
...
export function compatibleTableBenchmarkHistory(runnerClass: string): readonly TableBaselineHistoryEntry[] {
  const matches = tableBenchmarkBaseline.history.filter(entry => entry.runnerClass === runnerClass);
  if (matches.length === 0) throw new Error(`No table benchmark baseline matches runner class ${JSON.stringify(runnerClass)}; capture and review a baseline for this runner before applying performance gates`);

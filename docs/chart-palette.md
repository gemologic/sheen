...
For a focal metric with quieter comparisons, series may use `accent`, `foreground`, and `muted`. The latter two map to the actual scope's readable text tokens in SVG, legends, and the shared canvas token bridge. Use distinct `encoding` values and visible labels; this is an explicit per-chart choice, not a replacement for the qualified categorical palette. Theme changes redraw accepted data without remounting the chart.

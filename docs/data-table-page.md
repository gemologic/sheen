...
The admin starter's `/admin/accounts` route uses the same pattern for its collection. Application-wide commands remain in AdminApp's chrome; DataTable supplies its query toolbar directly on the page canvas. No empty application toolbar or status footer is required. Optional regions keep their own grid rows, so omitting them leaves the table in the remaining bounded space. `/admin/accounts?content=long` includes a long account name with a full, wrapping details heading reached by keyboard row activation.

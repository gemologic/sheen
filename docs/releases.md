...
`pnpm check:packages` runs `pnpm pack --dry-run --json` for every public package. It verifies repository and public-provenance metadata, fixed version alignment, all export/private-import and binary targets, package README and MIT license identity, and the absence of tests, demos, metadata declarations, local configuration, and lock files. The token package additionally proves that the three IBM Plex files, their pinned source record, and the SIL Open Font License are present. The UI package verifies vendored runtime hashes, original licenses/notices, and setup-plugin artifacts. The CLI package requires its generated skill and rejects consumer patch payloads. The command only inspects inventories and never creates a tarball or publishes.

## Packed npm consumer gate

Run `pnpm build && pnpm test:production && pnpm test:npm-consumer:browser` for a release candidate. The production gate starts the built Nitro application, asserts complete server-rendered HTTP responses and isolated cookie themes, then exercises delayed hydration with retained content, drafts, and node identity. The packed-consumer command creates real tarballs for all ten packages, installs those tarballs together with the pinned consumer toolchain into a clean temporary project through npm, verifies the complete dependency tree and high-severity audit boundary, typechecks the public entries, imports the Node-oriented token/CLI/lint entries, qualifies shared ESM/CommonJS SSR renderer identity and asynchronous request contexts, and builds both compiled and Solid-source browser graphs with Vite. It executes both production client conditions and development SSR/lazy hydration with the owned browser renderer and unpatched shared Solid core/server renderer. It removes the temporary project afterward and never contacts npm's publish endpoints. The build-only `pnpm test:npm-consumer` variant does not require Chromium.

This complements the copied-artifact and cross-engine application gates. Sheen owns the patched backend and renderer; there is no workspace patch requirement to transfer to consumers. Stable publication still fails if future workspace-only patches are introduced. The runtime browser contract must pass before any owned correction is removed or replaced.

## npm bootstrap and ongoing publication

For a new package family, start with the Changesets-generated fixed-version prerelease `0.1.0-rc.0` on the `next` tag. Changesets begins a new prerelease line at `.0`; do not hand-edit the generated version to make it `.1`. Review the version pull request, run all release gates, run the packed npm consumer gate, and inspect every generated tarball before publication. Brand-new package records use one-time bootstrap mode before their trusted-publisher relationships are configured. Use the `@gemologic` organization for scoped packages and `--access public`; `eslint-plugin-sheen` remains an intentionally unscoped global package name.
...
`pnpm build:packages && pnpm test:release-artifacts` exercises real Changesets stable and prerelease versioning, pack-and-verify, distribution-tag and publication-mode rejection, and tamper detection in a disposable checkout. It removes the checkout and tarballs afterward and never calls an npm publication endpoint.
...
After every package exists, configure one [npm trusted-publisher](https://docs.npmjs.com/trusted-publishers/) relationship per package with organization `gemologic`, repository `sheen`, workflow `publish.yml`, and environment `npm`. Allow direct `npm publish` for each relationship. Ongoing `publish` mode receives only `contents: read` and `id-token: write`, uses GitHub-hosted runners and npm OIDC, and has no npm token. One protected GitHub environment approval authorizes publication of all ten reviewed tarballs with provenance; there is no subsequent per-package npm approval. Keep account 2FA enabled and select "Require two-factor authentication and disallow tokens" in package publishing access. That setting does not prevent OIDC publishing.

Direct publishing relies on GitHub's workflow and environment approval instead of an independent npm proof-of-presence gate. Only approve a release after checking its source commit, requested version/tag, and prepared artifact manifest. The workflow defaults are `publish` and `latest`; a prerelease must explicitly use `next`, and the artifact gate rejects mismatched tags.

## Stable 0.1.0 promotion

Promote the release-candidate line by publishing a new stable `0.1.0` version, not by moving `latest` to an `rc` version. All ten packages keep one Changesets-owned version.

1. Synchronize with current `main`, then mark the [prerelease exit](https://changesets.dev/guide/prereleases#exit-prerelease-mode) with `pnpm changeset pre exit`. This changes `.changeset/pre.json` to `mode: "exit"`; it does not change package versions yet. The approved exit intent is already recorded in this change, so do not rerun that command for this promotion.
2. Push the exit intent and reviewed workflow changes. The credential-free Release PR workflow runs `pnpm version-packages` and opens a stable Version PR with package versions and changelogs. Review that it generates exactly `0.1.0` for every public package, then merge after its checks pass. Do not hand-edit versions or run another version bump on top of the generated PR.
3. Confirm the hosted Check, WebKit, benchmark, Pages, and packed npm-consumer gates for the exact stable release commit.
4. Dispatch **Publish npm packages** from `main` with version `0.1.0`, distribution tag `latest`, and publication mode `publish`. Review the prepared artifact before approving the protected `npm` environment once.
5. Verify all ten registry versions and `latest` tags, provenance, and clean consumer installation. `next` remains on its existing release candidate until a later prerelease is deliberately published there.
...
6. Configure the ten npm trusted publishers for `publish.yml`, environment `npm`, and direct `npm publish`. Use `publish` mode for later versions: prereleases use `next`, stable releases use `latest`.

...
import { createInteractOutside } from './XLBXJFYN.js';
import { layerStack } from './2YCOH52P.js';
import { Polymorphic } from './6Y7B2NEO.js';
import { createComponent, mergeProps } from 'solid-js/web';
import { getDocument, mergeRefs, contains } from "#sheen-kobalte-utils";
...
function DismissableLayer(props) {
  ...
  const onPointerDownOutside = (e) => {
    if (!ref || ref.hasAttribute("data-closed") || layerStack.isBelowPointerBlockingLayer(ref)) {
      return;
    }
  ...
  const onFocusOutside = (e) => {
    if (!ref || ref.hasAttribute("data-closed")) return;

...
import { getWindow, mergeDefaultProps, mergeRefs } from "#sheen-kobalte-utils";
import {
  createEffect,
  createSignal,
  splitProps
...
function PopperRoot(props) {
  ...
  createEffect2(() => {
    ...
    let disposed = false;
    const syncZIndex = () => { if (!disposed) positioner.style.zIndex = getComputedStyle(content).zIndex; };
    queueMicrotask(syncZIndex);

...
import { access, mergeDefaultProps } from "#sheen-kobalte-utils";
...
import { isAppleDevice, isMac } from "#sheen-kobalte-utils";
...
import { access as access2 } from "#sheen-kobalte-utils";
...
function createSelectableCollection(props, ref, scrollRef) {
  ...
  const tryAutoFocus = () => {
    ...
    const manager = access3(mergedProps.selectionManager);
    const container = ref();
    if (manager.isFocused() && manager.focusedKey() != null && container?.contains(container.ownerDocument.activeElement)) return;
    const delegate = access3(mergedProps.keyboardDelegate);
    let focusedKey;
    if (autoFocus === "first") {
...
import { access as access5 } from "#sheen-kobalte-utils";

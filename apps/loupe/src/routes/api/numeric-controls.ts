export async function GET(event: { request: Request }): Promise<Response> {
  ...
  if (!Number.isSafeInteger(revision) || revision < 0 || !Number.isSafeInteger(delay) || delay < 0 || delay > 5_000) return new Response("Invalid numeric control request", { status: 400 });

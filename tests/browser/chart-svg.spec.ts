...
test("sparkline gaps and stat trends have semantic non-color output", async ({ page }) => {
  ...
  const colors = await group.locator('[data-trend="down"]').evaluate(element => {
    ...
    return { resolved: pixel(style.color), token: pixel(style.getPropertyValue("--sheen-color-market-down").trim()) };
  });
...
test("real refresh keeps accepted SVG and stat roots nonblank until atomic acceptance", async ({ page }) => {
  ...
  const samples = await page.locator(".loupe-chart-svg-surface").evaluate(async surface => {
    const frames: Array<{ readonly sparkline: boolean; readonly stats: boolean; readonly table: boolean; readonly row: boolean; readonly path: string }> = [];
  ...
  expect(samples.every(sample => sample.sparkline && sample.stats && sample.table && sample.row && sample.path === before)).toBe(true);
